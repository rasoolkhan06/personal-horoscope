...
export class HoroscopeService {
  ...
  async updateHoroscope(
    id: string,
    updateData: Partial<CreateHoroscopeDto>,
  ): Promise<IHoroscope> {
    try {
      if (!Types.ObjectId.isValid(id)) {
        throw new BadRequestException('Invalid horoscope ID');
      }

      const updated = await this.horoscopeModel
      ...
      throw new BadRequestException('Failed to update horoscope');
    }
  }

  async deleteHoroscope(id: string): Promise<{ success: boolean }> {
    try {
      if (!Types.ObjectId.isValid(id)) {
        throw new BadRequestException('Invalid horoscope ID');
      }

      const result = await this.horoscopeModel.deleteOne({ _id: id }).exec();
      ...
      throw new BadRequestException('Failed to delete horoscope');
    }
  }

  async getHoroscopeById(id: string): Promise<IHoroscope> {
    try {
      if (!Types.ObjectId.isValid(id)) {
        throw new BadRequestException('Invalid horoscope ID');
      }
      ...
      throw new BadRequestException('Failed to fetch horoscope');
    }
  }
}
